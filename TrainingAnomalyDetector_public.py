...
#    print("Loading training batch")
    ...
    Num_Normal = 751    # Total number of Normal videos in Training Dataset.
...
AllClassPath='/content/drive/My Drive/AnomalyDetectionCVPR2018/SampleVideos/'
# AllClassPath contains C3D features (.txt file)  of each video. Each text file contains 32 features, each of 4096 dimension
output_dir='/content/drive/My Drive/AnomalyDetectionCVPR2018/Trained_Models/TrainedModel_MIL_C3D/'
...
for it_num in range(num_iters):
    ...
    AbnormalPath = os.path.join(AllClassPath, All_class_files[1])  # Path of abnormal already computed C3D features
    NormalPath = os.path.join(AllClassPath, All_class_files[3])    # Path of Normal already computed C3D features
    print(AbnormalPath+NormalPath)
